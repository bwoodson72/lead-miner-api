...
}): Promise<{ draft: OutreachDraft; model: string; inputTokens?: number; outputTokens?: number }> {
  const env = getEnv();
  if (!env.OPENAI_API_KEY) throw new Error("OPENAI_API_KEY is not configured");
  const model = env.OPENAI_OUTREACH_MODEL;
  const packet = {
    ...
    problems: input.problems.filter((p) => p.confidence >= 0.7 && p.outreachValue !== "low").slice(0, 4),
  };
