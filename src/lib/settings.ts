...
export const AppSettingsInputSchema = z.object({
  ...
  gmailRolling24hSafetyLimit: z.number().int().min(1).max(2000),
  minimumSendIntervalMinutes: z.number().int().min(1).max(1440),
  gmailQuotaCooldownHours: z.number().int().min(1).max(72),
