...
export function registerOutreachRoutes(app: Express, prisma: PrismaClient) {
  ...
  app.get("/api/outreach/review", async (_req, res) => {
    try {
      const messages = await prisma.outreachMessage.findMany({
        ...
        include: { lead: { include: { problems: { orderBy: { confidence: "desc" }, take: 4 }, scores: { orderBy: { createdAt: "desc" }, take: 1 } } } },
      });
  ...
  app.post("/api/outreach/backfill-drafts", async (_req, res) => {
    try {
      const leads = await prisma.lead.findMany({
        where: { qualificationDecision: "qualified", email: { not: null }, status: { in: ["qualified", "ready_for_outreach"] }, outreachMessages: { none: { kind: "initial", sequenceNumber: 1, status: { in: ["draft", "approved", "sent"] } } } },
        orderBy: [{ priorityScore: { sort: "desc", nulls: "last" } }, { createdAt: "desc" }], take: 25, select: { id: true },
      });
  ...
  app.patch("/api/outreach/:id", async (req, res) => {
    ...
    if (status !== undefined) {
      ...
      data.status = String(status); if (status === "approved") data.approvedAt = new Date(); if (status === "draft") data.approvedAt = null;
    }
    try {
      const updated = await prisma.$transaction(async (tx) => {
        const message = await tx.outreachMessage.update({ where: { id }, data });
        await tx.activity.create({ data: { leadId: message.leadId, type: status === "approved" ? "message_approved" : status === "rejected" ? "message_rejected" : "message_updated", summary: status === "approved" ? "Initial outreach approved" : status === "rejected" ? "Outreach draft rejected" : "Outreach draft edited" } });
        return message;
      });
      res.json(updated);
    } catch (error) { res.status(500).json({ error: error instanceof Error ? error.message : String(error) }); }
