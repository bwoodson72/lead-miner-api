...
import { normalizeRegenerationInstruction, withRegenerationInstruction } from "./outreach-regeneration-guidance.js";
...
const RegenerateSchema = z.object({
  instruction: z.string().max(1000).nullable().optional(),
...
export function registerOutreachRoutes(app: Express, prisma: PrismaClient) {
  ...
  app.post("/api/outreach/:id/regenerate", async (req, res) => {
    ...
    const legacyReason = parsed.reason?.trim();
    const instruction = normalizeRegenerationInstruction(
      parsed.instruction ?? (legacyReason && legacyReason !== "Regenerate with current outreach rules" ? legacyReason : null),
    );
    try {
      const regeneration = await withRegenerationInstruction(instruction, () => regenerateUnsentOutreach(prisma, { scope: "initial", messageIds: [id], limit: 1, force: true }));
      ...
      const generationReason = instruction ? `Operator instruction: ${instruction}` : "Regenerated by operator request";
      const message = outcome.replacementMessageId
        ? await prisma.outreachMessage.update({ where: { id: outcome.replacementMessageId }, data: { generationReason } })
        : null;
      if (message) {
        await prisma.activity.create({
          data: {
            ...
            summary: instruction ? "Outreach regenerated with operator instruction" : "Outreach regenerated with current rules",
            metadata: { previousMessageId: id, replacementMessageId: message.id, instruction },
          },
        });
      }
      res.json({ message, outcome, instructionApplied: Boolean(instruction) });
    } catch (error) { res.status(409).json({ error: error instanceof Error ? error.message : String(error) }); }
  });
