...
export async function ensureInitialOutreachDraft(prisma: PrismaClient, leadId: number) {
  const env = getEnv();
  if (!env.AUTO_DRAFT_OUTREACH) return null;
  ...
  const aiJob = await prisma.aIJob.create({
    data: { leadId, type: "outreach_draft", status: "running", model: env.OPENAI_OUTREACH_MODEL, promptVersion: OUTREACH_PROMPT_VERSION, startedAt: new Date() },
  });
  ...
  try {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      await tx.activity.create({ data: { leadId, type: "message_generated", summary: `Initial outreach draft generated: ${generated.draft.subject}`, metadata: { confidence: generated.draft.confidence, model: generated.model } } });
...
export async function processLeadResearch(prisma: PrismaClient, leadId: number) {
  ...
  const env = getEnv();
  const job = await prisma.aIJob.create({ data: { leadId, type: "lead_research", status: "running", model: env.OPENAI_RESEARCH_MODEL, promptVersion: RESEARCH_VERSION, startedAt: new Date() } });
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (result.problems.length) {
        await tx.leadProblem.createMany({ data: result.problems.map((p) => ({ leadId, category: p.category, title: p.title, evidence: p.evidence, businessConsequence: p.businessConsequence, recommendedImprovement: p.recommendedImprovement || null, confidence: p.confidence, outreachValue: p.outreachValue })) });
      }
...
export function registerResearchRoutes(app: Express, prisma: PrismaClient) {
  ...
  app.post("/api/leads/:id/research", async (req, res) => {
    ...
    try {
      ...
      const updated = await prisma.lead.findUnique({ where: { id }, include: { problems: true, scores: { orderBy: { createdAt: "desc" }, take: 1 }, outreachMessages: { orderBy: { generatedAt: "desc" } } } });
      res.json(updated);
  ...
  app.post("/api/leads/bulk-research", async (req, res) => {
    ...
    const leads = requested.length
      ? await prisma.lead.findMany({ where: { id: { in: requested } }, take: env.AI_RESEARCH_BATCH_SIZE, select: { id: true } })
      : await prisma.lead.findMany({ where: { status: { in: ["new", "research_pending"] } }, orderBy: { createdAt: "asc" }, take: env.AI_RESEARCH_BATCH_SIZE, select: { id: true } });
