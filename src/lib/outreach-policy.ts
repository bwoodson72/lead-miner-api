...
export const OUTREACH_VALIDATION_MESSAGES = {
  senderIdentity: "Sender identity in the body is optional; do not add a bio or service sentence solely to identify Brian.",
...
export function buildHardOutreachRules() {
  ...
  return [
    "Write Touch 1 as a short email Brian would personally type after noticing something real on a business website.",
    ...
    "The lead reaching this stage has been qualified because a custom rebuild is a reasonable business option. Touch 1 still should not pitch the rebuild; use the supplied context to earn a reply first.",
    ...
    "Use the selected research finding as the primary evidence-backed observation and connect it to one owner stake. If operatorContext contains a concrete, relevant firsthand observation from My Notes, incorporate one such operator observation into the email alongside the selected finding unless doing so would violate a factual or safety rule. Do not ignore a relevant operator observation merely because the selected research finding is about a different website weakness. Operator observations supplement message wording only; they do not replace the selected finding, select a new outreach angle, or change qualification or priority.",
    ...
    "Do not copy or lightly paraphrase private strategy or operator-note wording. Use relevant operator observations naturally, as if Brian personally noticed them, without exposing My Notes or calling them notes.",
    "Never invent traffic loss, lead loss, revenue loss, ad spend, rankings, urgency, customer behavior, or business plans. Imagined customer behavior must remain a possibility, never a known event. Operator notes may supply concrete firsthand observations, but speculative claims in notes must remain speculative and must never be upgraded into known facts.",
    ...
    "Do not force a sender bio, credentials sentence, or statement that Brian builds websites. The first-name signoff is sufficient identification for Touch 1. Mention what Brian does only when it genuinely makes the email clearer and still reads like a personal heads-up rather than a sales introduction.",
    ...
    "If regeneration.previousDraft is present, produce a materially different email rather than a light paraphrase. Change the opening construction, sentence structure, body sequence or framing, and CTA wording while preserving the same supported facts and Touch 1 objective. Do not reuse a canned sender-introduction pattern from the previous draft.",
