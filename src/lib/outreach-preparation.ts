...
export async function ensureInitialOutreachDraft(prisma: PrismaClient, leadId: number, force = false) {
  ...
  const reuseExisting = existing ? canReuseExistingInitialOutreach(existing, force || notesChangedAfterDraft) : false;
  ...
  try {
    ...
    return await prisma.$transaction(async (tx) => {
      if (replaceExisting && existing) {
        await tx.outreachMessage.update({ where: { id: existing.id }, data: { status: "cancelled", requiresReview: true, sendError: notesChangedAfterDraft ? "Superseded because operator outreach notes changed" : "Superseded because the stored draft was stale or failed current Touch 1 rules" } });
      }
