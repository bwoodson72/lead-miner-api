...
export function canReuseExistingInitialOutreach(existing: {
  status: string;
  ...
  if (["sending", "sent"].includes(existing.status)) return true;
  if (force) return false;
  return existing.promptVersion === OUTREACH_PROMPT_VERSION
...
export async function ensureInitialOutreachDraft(prisma: PrismaClient, leadId: number, force = false) {
  ...
  const reuseExisting = existing ? canReuseExistingInitialOutreach(existing, force) : false;
  ...
  try {
    ...
    return await prisma.$transaction(async (tx) => {
      if (replaceExisting && existing) {
        await tx.outreachMessage.update({ where: { id: existing.id }, data: { status: "cancelled", requiresReview: true, sendError: "Superseded because the stored draft was stale or failed current Touch 1 rules" } });
      }
      const message = await tx.outreachMessage.create({
        data: {
          ...
          generationReason: force ? "Regenerated by operator request" : replaceExisting ? "Regenerated because stored draft was stale or failed current rules" : null,
          promptVersion: OUTREACH_PROMPT_VERSION,
          status,
