...
export const OUTREACH_ANGLE_PROMPT_VERSION = "outreach-angle-v1";
...
export async function selectOutreachAngle(input: {
  ...
  findings: Array<{ id: number; category: string; title: string; evidence: string; assetCapability: string; confidence: number; significance: string; evidenceSources: unknown }>;
}, model: string) {
  ...
  const hardRules = [
    ...
    "Prefer a finding that is concrete, consequential to the website as a business asset, high-confidence, and naturally aligned with custom web development or optimization.",
    ...
    "Do not write an email, CTA, subject line, or sales script. Only choose the evidence-backed angle and explain internally why it is the best one.",
  ...
  if (!input.findings.some((finding) => finding.id === result.findingId)) throw new Error("OpenAI selected a finding that was not supplied");
