...
test("stale research refresh upgrades pre-v13 research and reports invalidated unsent outreach", async () => {
  ...
  assert.equal(result.targetResearchVersion, "lead-research-v13");
  assert.deepEqual(result.staleResearchVersions, ["lead-research-v3", "lead-research-v4", "lead-research-v5", "lead-research-v6", "lead-research-v7", "lead-research-v8", "lead-research-v9", "lead-research-v10", "lead-research-v11", "lead-research-v12"]);
...
test("v13 refresh does not generate replacement outreach before prioritization and angle selection", async () => {
  const result = await refreshStaleResearch(
    makePrisma("lead-research-v12"),
    10,
    (async () => ({
